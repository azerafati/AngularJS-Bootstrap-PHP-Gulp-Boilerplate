...
class CategoryControllerAdmin {
	...
	static function loadAll($_req) {
		$categories = CategoryRepository::loadAll(false, false, 'order by sort asc,id asc');
		$cats = [];
		foreach ($categories as $cat) {
			$exportCat = new stdClass();
			$exportCat->id = intval($cat->id);
			$exportCat->name = $cat->title;
			$exportCat->parent_id = ($cat->parent_id);
			$cats[] = $exportCat;
		}
